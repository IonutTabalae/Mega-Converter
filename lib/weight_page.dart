...
class _WeightPageState extends State<WeightPage> {
  ...
  void _setLastConversion() {
    final double? lastInputValue = _prefs?.getDouble('weight_inputValue');
    final String? lastSelectedInputUnit =
        _prefs?.getString('weight_selectedInputUnit');
    final String? lastSelectedOutputUnit =
        _prefs?.getString('weight_selectedOutputUnit');
    final double? lastConvertedValue =
        _prefs?.getDouble('weight_convertedValue');
...
// Add conversion functionality here
  ...
  void _convertUnits() {
    ...
    var inputUnit = MASS.values.firstWhere((unit) =>
        unit.name ==
        selectedInputUnit.toLowerCase().replaceAll(RegExp(r'\s+'), ''));
    var outputUnit = MASS.values.firstWhere((unit) =>
        unit.name ==
    ...
    _prefs?.setDouble('weight_inputValue', inputValue);
    _prefs?.setString('weight_selectedInputUnit', selectedInputUnit);
    _prefs?.setString('weight_selectedOutputUnit', selectedOutputUnit);
    _prefs?.setDouble('weight_convertedValue', convertedValue ?? 0);
  }
}
